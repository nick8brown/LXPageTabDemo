...
@interface LXPageTabView () <UIScrollViewDelegate>
//bg
...
- (void)layoutSubviews {
    if(_isNeedRefreshLayout) {
        ...
        for(NSInteger i = 0; i < _tabItems.count; i++) {
            LXPageTabItemLable *tabItem = (LXPageTabItemLable *)_tabItems[i];
            tabItem.frame = CGRectMake(_tabItemWidth*i, 4, _tabItemWidth, _tabSize.height-_indicatorHeight-4);
        }
...
- (void)initBaseSettings {
    ...
    _isChangeByClick = NO;
    _bodyBounces = YES;
    _titleStyle = LXPageTabTitleStyleDefault;
    _indicatorStyle = LXPageTabIndicatorStyleDefault;
    _minScale = kMinScale;
    _selectedColorR = 1;
    _selectedColorG = 0;
    _selectedColorB = 0;
...
- (void)initTabView {
    ...
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        LXPageTabItemLable *tabItem = [[LXPageTabItemLable alloc] init];
...
- (void)reviseTabContentOffsetBySelectedIndex:(BOOL)isAnimate {
    LXPageTabItemLable *currentTabItem = _tabItems[_selectedTabIndex];
...
- (void)changeSelectedItemToNextItem:(NSInteger)nextIndex {
    LXPageTabItemLable *currentTabItem = _tabItems[_selectedTabIndex];
    LXPageTabItemLable *nextTabItem = _tabItems[nextIndex];
...
- (void)resetTabItemScale {
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        LXPageTabItemLable *tabItem = _tabItems[i];
...
- (void)addIndicatorViewWithStyle {
    switch (_indicatorStyle) {
        case LXPageTabIndicatorStyleDefault:
        case LXPageTabIndicatorStyleFollowText:
        case LXPageTabIndicatorStyleStretch:
...
//    CGFloat indicatorWidth = [self getIndicatorWidthWithTitle:_childTitles[_selectedTabIndex]];
    LXPageTabItemLable *selecedTabItem = _tabItems[_selectedTabIndex];
    self.indicatorView.frame = CGRectMake(selecedTabItem.center.x-_indicatorWidth/2.0-_tabView.contentOffset.x, _tabSize.height-_indicatorHeight+1, _indicatorWidth, _indicatorHeight);
}
...
- (void)changeChildControllerOnClick:(UITapGestureRecognizer *)tap {
    ...
    if(nextIndex != _selectedTabIndex) {
        if(_titleStyle == LXPageTabTitleStyleDefault) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if(scrollView == self.tabView) {
        ...
        //调整title
        switch (_titleStyle) {
            case LXPageTabTitleStyleDefault:
                [self changeTitleWithDefault];
                break;
            case LXPageTabTitleStyleGradient:
                [self changeTitleWithGradient];
                break;
            case LXPageTabTitleStyleBlend:
...
- (void)changeTitleWithGradient {
    if(_leftItemIndex != _rightItemIndex) {
        ...
        LXPageTabItemLable *leftTabItem = _tabItems[_leftItemIndex];
        LXPageTabItemLable *rightTabItem = _tabItems[_rightItemIndex];
...
//    CGFloat nowIndicatorWidth = leftIndicatorWidth + (rightIndicatorWidth-leftIndicatorWidth)*relativeLocation;
    ...
    self.indicatorView.frame = CGRectMake(nowIndicatorCenterX-WIDTH(self.indicatorView)/2.0-_tabView.contentOffset.x, ORIGIN_Y(self.indicatorView), WIDTH(self.indicatorView), HEIGHT(self.indicatorView));
}
...
- (CGFloat)getIndicatorWidthWithTitle:(NSString *)title {
    if(_indicatorStyle == LXPageTabIndicatorStyleDefault || _indicatorStyle == LXPageTabIndicatorStyleStretch) {
...
- (UIImageView *)indicatorView {
    if(!_indicatorView) {
        ...
        _indicatorView.image = [UIImage imageNamed:@"三角形"];
    }
...
- (void)setSelectedTabIndex:(NSInteger)selectedTabIndex {
    if(selectedTabIndex >= 0 && selectedTabIndex < _numberOfTabItems && _selectedTabIndex != selectedTabIndex) {
        ...
        if(_titleStyle == LXPageTabTitleStyleGradient) {
...
- (void)setSelectedColor:(UIColor *)selectedColor {
    ...
    LXPageTabItemLable *tabItem = _tabItems[_selectedTabIndex];
...
- (void)setTitleStyle:(LXPageTabTitleStyle)titleStyle {
    if(_titleStyle == LXPageTabTitleStyleDefault) {
...
- (void)setIndicatorStyle:(LXPageTabIndicatorStyle)indicatorStyle {
    if(_indicatorStyle == LXPageTabIndicatorStyleDefault) {
